...
root = Tk()
root.title('Кулькулятор уровней для PayDay 2')
root.geometry('450x57')
root.resizable(0, 0)
...
def calc():
    ...
    try:
        n = str(calc_2(a,b))[::-1]
        res = ('.'.join(n[i:i + 3] for i in range(0, len(n), 3))[::-1])
    except:
        res = 'Ошибка'
...
vop1 = Label(root, text='Какой уровень сейчас или сколько опыта у вас имеется?')
vop1.place(x=5, y=5)
en1 = Entry(root, width=12)
en1.place(x=325,y=7)
vop2 = Label(root, text='Какой уровень вы хотите?')
vop2.place(x=5, y=27)
en2 = Entry(root, width=5)
en2.place(x=157,y=29)
but = Button(root, text='Скулькулировать', command=calc).place(x=200,y=25)
result = Label(root, text='Результат', bg='white')
result.place (x=320,y=29)
root.mainloop()
